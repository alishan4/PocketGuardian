...
def register_user():
    ...
    try:
        ...
        db.collection('users').document(user_id).set({
            ...
            'email': data.get('email', ''),
            'fcm_token': data.get('fcm_token', ''),
            'weekly_limit': data.get('weekly_limit', 10000.0),  # Default weekly limit
            'monthly_limit': data.get('monthly_limit', 100000.0)  # Default monthly limit
        })
        return jsonify({'success': True})
    except Exception as e:
        logging.error(f"User registration failed: {str(e)}")
...
def mobile_user_summary(user_id):
    ...
    if db:
        try:
            # Load user limits from user doc
            user_ref = db.collection("users").document(user_id)
            user_doc = user_ref.get()
            if user_doc.exists:
                user_data = user_doc.to_dict()
                summary["summary"]["weekly_limit"] = user_data.get("weekly_limit", 10000.0)
                summary["summary"]["monthly_limit"] = user_data.get("monthly_limit", 100000.0)
            ...
            for doc in transactions:
                ...
                if hasattr(tstamp, "to_datetime"):
                    dt = tstamp.to_datetime().date()
            ...
            summary["summary"]["today_spent"] = today_spent
            summary["summary"]["weekly_spent"] = weekly_spent
            summary["summary"]["monthly_spent"] = monthly_spent
            summary["summary"]["transactions_today"] = transactions_today

            # Count fraud alerts for this user
            alerts_ref = db.collection("alerts").where("user_id", "==", user_id).where("type", "==", "fraud")
            fraud_alerts = len(list(alerts_ref.stream()))
            summary["summary"]["fraud_alerts"] = fraud_alerts
...
def analyze_sms_full():
    ...
    try:
        ...
        # Step 1: Parse SMS
        parsed = parse_sms(sms_text)
        logger.info(f"Parsed SMS: {parsed}")
        ...
        if fraud['fraud']:
            ...
            db.collection('alerts').document(alert_id).set({
                'user_id': user_id,
                'type': 'fraud',
                'message': '⚠️ Fraudulent SMS detected! This may be a scam.',
                'data': fraud,
                'timestamp': firestore.SERVER_TIMESTAMP,
                'read': False
            })

            # Create reminder for fraud
            reminder_id = f"{user_id}_fraud_reminder_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            db.collection('reminders').document(reminder_id).set({
                'user_id': user_id,
                'message': 'Potential fraud detected in SMS. Please verify.',
                'due_date': datetime.now().strftime('%Y-%m-%d'),
                'sent': False
            })
            reminder_created = True
            ...
            # Not fraud → Store as valid transaction if it's a transaction (amount > 0)
            if parsed.get("amount", 0) > 0:
                ...
                # Step 3: Overspending detection (includes limit checks)
                overspending = detect_overspending(user_id, db)
                db.collection('agent_logs').add({
                    'user_id': user_id,
                    'agent_name': 'ML Alert',
                    'action': 'Checked overspending',
                ...
                if overspending.get("overspending", False):
                    alert_id = f"{user_id}_overspending_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    db.collection('alerts').document(alert_id).set({
                        'user_id': user_id,
                        'type': 'overspending',
                        'message': '⚠️ Overspending detected! Review your expenses.',
                        'data': overspending,
                        'timestamp': firestore.SERVER_TIMESTAMP,
                        'read': False
                    })

                    # Create reminder for overspending
                    reminder_id = f"{user_id}_overspending_reminder_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    db.collection('reminders').document(reminder_id).set({
                        'user_id': user_id,
                        'message': 'You are overspending. Check your weekly/monthly limits.',
                        'due_date': datetime.now().strftime('%Y-%m-%d'),
                        'sent': False
                    })
                    reminder_created = True
